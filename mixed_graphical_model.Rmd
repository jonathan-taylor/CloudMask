...
```{python}
groups = list(groups)
penalty = rr.group_lasso(groups, weights=weights, lagrange=1.2e4) 
penalty.shape
```
...
H[loss.matrix_slice][:,loss.matrix_slice] = (np.kron(I, XTX) + np.kron(XTX, I))
...
param_grid = {'atom_params': [{'groups':groups,
                              'weights':weights,
                              'lagrange':lamval} for lamval in 1e3 * np.linspace(5, 15, 11)]}
